    ...
    public function verify(Request $request, Authenticatable $user, AuthyApi $authyApi, Client $client)
    {
        ...
        if ($token) {
            ...
            if ($verification->ok()) {
                ...
                $emailAdmin=$user::where('role_id',4)->select('email');
                // $this->sendSmsNotification($client, $user);
                Mail::send('mail', array("name" => '', "email" => '', "content" => 'Bạn đã đăng ký làm đối tác với HotelBooking'), function ($message) use ($email) {
                    $message->to($email)->subject('Thông báo đăng ký!')->from('doducan.ltu14@gmail.com', 'HotelBooking')
                        ->sender('doducan.ltu14@gmail.com', 'HotelBooking');
                });
//                Mail::send('mail', array("name" => '', "email" => '', "content" => 'Đăng ký làm đối tác với OYO.com'), function ($message) use ($emailAdmin) {
//                    $message->to($emailAdmin)->subject('Thông báo đăng ký!')->from('doducan.ltu14@gmail.com', 'OYO.com')
//                        ->sender('doducan.ltu14@gmail.com', 'OYO.com');
//                });
