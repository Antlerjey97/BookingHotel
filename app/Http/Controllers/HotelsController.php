...
class HotelsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (empty($Searchterm) && empty($NumTravelers)) {
            $Hotels = Hotel::where('status', '<=', 2)->get();
            ...
            $request->session()->put('Capacity', $NumTravelers);
    ...
    {
//        dd($request->session()->get('Capacity'));
        ...
        if($capacity){
            $Rooms = $hotel->rooms()->where('Capacity', '>=', $capacity)->get();
        }
        else {
            $Rooms = $hotel->rooms;
        }
        ...
        return view('hotels.hoteldetails', compact('hotel', 'Rooms', 'Photos', 'Rating', 'starPath', 'RecentBooking'));
    }
    ...
    {
        $rooms = $hotel->rooms();
        return view('partners.actions', compact('hotel','rooms'));
    }
    ...
    public function edit( Partner $partner,$id)
    {
        ...
        $array = ['Ninh Bình','Ninh Hải ', 'Ninh Xuân', 'Trường Yên' ,
                 'Gia Sinh',];
